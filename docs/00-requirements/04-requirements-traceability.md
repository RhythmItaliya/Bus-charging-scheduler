...
| R11 20 buses | 03/02 | scenario files | `test_e2e`, `test_loader` |
| R12 departure | 03/02 | `Bus.departure_min` | `test_e2e`, `test_loader` |
| R13 operators data-driven | 03/01 | derived set | `test_e2e`, `test_loader` |
| R14 range per leg | 00/02 H1 | `RangeRule` | `test_plans`, `test_rules`, `test_validate` (H1) |
| R15 ≥2 charges | 00/01 §4 | plan generator | `test_plans`, `test_e2e`, `test_validate` (R15) |
| R16 charger exclusivity | 00/02 H3 | `ChargerPool` + validator | `test_charger`, `test_validate` (H3) |
| R17 25-min charge | 00/02 H4 | `ChargeDurationRule` | `test_rules`, `test_validate` (H4) |
| R18 never run out | 00/02 H1 | `RangeRule` + validator | `test_e2e`, `test_rules`, `test_validate` |
| R19 route order | 00/02 H2 | `RouteOrderRule` | `test_plans`, `test_rules`, `test_validate` (H2) |
...
| R25 grow world | 03/01, 00/03 | data-driven model | live curveballs, `test_loader` |
| R26 data structure first | 03/01 | scenario schema | review |
| R28 S4 operator=2.0 | 03/02 | `scenario_4.json` | `test_weights` |
| R29 read scenario | 06/01 | `loader.load_scenario` | `test_e2e`, `test_loader` (50 validation tests) |
| R30 plan + order | 02/01 | `engine.schedule` | `test_e2e` |
| R31 per-bus timeline | 02/01 | `BusPlan` | `test_e2e`, `test_adapters` |
| R32 dropdown first | 05/01 | `app.py` selectbox | hosted smoke |
| R33 input view | 05/02 | `to_input_table` | `test_adapters` (columns, HH:MM, sort) |
| R34 per-bus timetable | 05/02 | `to_bus_table` | `test_adapters` (wait, arrival, total_wait) |
| R35 per-station view | 05/02 | `to_station_table` | `test_adapters` (order, charger #, format) |
...
| R44 correctness | 07/01 | validator + test suite | `test_e2e`, `test_weights`, `test_rules`, `test_validate` |
...
| `test_loader.py` | 50 | Full 3-stage input validation (world, route, stations, weights, buses) |
| `test_rules.py` | 23 | H1/H2/H4 violating+satisfying; S1/S2/S3 penalty+weight scaling |
| `test_validate.py` | 22 | Post-schedule validator: each hard rule triggered by crafted data |
| `test_adapters.py` | 31 | DataFrame adapters: columns, HH:MM, row counts, per-bus summaries |
| `test_e2e.py` | 38 | All 5 scenarios: validate==[], ≥2 charges, arrival, count, objective, determinism |
| `test_weights.py` | 4 | R28/R44: operator weight changes breakdown + score |
| `conftest.py` | — | Shared fixtures only — no test methods |
